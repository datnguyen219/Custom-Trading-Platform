...
from app.models import Stock, Holding, db, User, BuyLimitOrder
from app.forms import HoldingForm

holding_routes = Blueprint("holdings",__name__)

def handle_holding(user_id, symbol, shares):
    ...
    if found_holding is not None:
        found_holding.shares += holding.shares
    else:
...
def post_holding():
    ...
    if form.validate_on_submit() and symbol is not None:
        user_data = handle_holding(user_id, symbol, form.data['shares'])
        return user_data
    else:
        user = User.query.get(user_id)
        return user.to_dict()
